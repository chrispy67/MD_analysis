...
def main(**kwargs):
    ...
    if not rc_max == float(pattern.findall(data_files[-1])[0]) or not rc_min == float(pattern.findall(data_files[0])[0]):
        # for file in glob.glob(input_path + 'rcwin_*_us.dat'):
        #     # print(file)
        #     # print(float(pattern.findall(file)[0]))
        data_files = [name.replace(input_path, '') for name in data_files if rc_min <= float(pattern.findall(name)[0]) <= rc_max]
        data_files = sorted(data_files, key=lambda x: float(pattern.findall(x)[0]))
...
#####ACCOUNTING FOR DIFFERENT KAPPA VALUES IN EXTRA WINDOWS
            #rcwin_extra = [5.064285714285714,5.104761904761904,5.338392857142856,5.445535714285713]
            #if center in rcwin_extra:
            #    kconst = 47801
    ...
    #K_k = []
    rcwin_extra = [5.064285714285714,5.104761904761904,5.338392857142856,5.445535714285713, 4.9767855, 5.0125, 5.182142857142857, 5.21190476190476,5.247619047619047]
    ...
    # centers = [0.08267349088394192*i for i in range(20)]
    # for i in range(20):
    #     centers.append(0.41329397*i)
    # print(centers)
    ...
    with open(kwargs['o'][0], 'a') as f:
        ...
        for this_data in alldata:
            ...
            n, bins, null = ax.hist(this_data, nbins, color=nextcolor)
    ...
    if not kwargs['quiet']:
        ...
        verbose = False
    mbar = pymbar.mbar.MBAR(u_kln, N_k, verbose=verbose, maximum_iterations=1500)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-k', metavar='kconst', type=float, nargs=1, default=[50.0], #this is kcal/mol 
